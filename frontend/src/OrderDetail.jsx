...
const OrderDetail = () => {
  ...
  const handlePayFullAmount = async () => {
    if (!hasCustomizedItems(order)) {
      alert("This feature is only for customized orders.");
      return;
    }
    ...
    if (!window.confirm(`Complete payment for the remaining balance of ₱${remainingAmount.toLocaleString()}?`)) {
      return;
    }

    setStatusUpdateLoading(true);
    try {
      console.log("Creating balance payment session:", {
        orderId: order._id,
                    ...
                    <div className="bg-light p-3 rounded">
                      ...
                      <div className="small">
                        ...
                        <div className="d-flex justify-content-between">
                          ...
                          <span className="text-success">₱{(order.paidAmount || 0).toLocaleString()}</span>
                        </div>
                        {getRemainingBalance(order) > 0 && (
                          <div className="d-flex justify-content-between">
                            <span>Remaining Balance:</span>
                            <span className="text-warning fw-bold">₱{getRemainingBalance(order).toLocaleString()}</span>
                          </div>
                        )}
                        <hr className="my-2" />
                  ...
                  <Col md={6}>
                      ...
                      <ListGroup.Item className="px-0 py-2 border-0">
                        <div className="d-flex justify-content-between">
                          ...
                          <span className="fw-medium">{order.deliveryOption === 'shipping' ? 'Shipping' : 'Pickup'}</span>
                        </div>
                      </ListGroup.Item>
                    </ListGroup>
                  </Col>
