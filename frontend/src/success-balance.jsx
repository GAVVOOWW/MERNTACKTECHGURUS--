  ...
  useEffect(() => {
    ...
    const processBalancePayment = async () => {
      try {
        ...
        // First try to manually confirm the balance payment
        console.log("=== MANUALLY CONFIRMING BALANCE PAYMENT ===");
        try {
          const confirmResponse = await axios.post(`${BACKEND_URL}/api/orders/${orderId}/confirm-balance-payment`, {
            orderId: orderId
          }, {
            headers: { Authorization: `Bearer ${token}` }
          });
          console.log("Balance payment confirmed:", confirmResponse.data);
        } catch (confirmError) {
          console.warn("Manual balance payment confirmation failed:", confirmError.message);
        }
        ...
        if (order.paymentStatus === 'Fully Paid' && (order.balance === 0 || order.balance === null)) {
          ...
          // Force update the order status if we're on the success page
          setSuccessMessage('Balance payment completed successfully! Your order is now fully paid.');
          // Update the local order object to show correct status
          if (order.paymentStatus === 'Downpaid') {
          ...
          <div className="bg-light rounded p-4 mb-4">
            ...
            <div className="row text-start">
              <div className="col-sm-6">
                ...
                <p><strong>Remaining Balance:</strong> <span className={orderDetails.balance === 0 ? 'text-success' : 'text-warning'}>₱{(orderDetails.balance || 0).toLocaleString()}</span></p>
              </div>
              <div className="col-sm-6">
                <p><strong>Status:</strong> <span className="badge bg-success">{orderDetails.status}</span></p>
                <p><strong>Payment Status:</strong> <span className={`badge ${orderDetails.paymentStatus === 'Fully Paid' ? 'bg-success' : 'bg-warning'}`}>{orderDetails.paymentStatus}</span></p>
              </div>
            </div>
            <div className="text-center mt-3">
              <p className="text-success mb-0">
                <i className="fas fa-check-circle me-2"></i>
                {orderDetails.paymentStatus === 'Fully Paid' && orderDetails.balance === 0 
